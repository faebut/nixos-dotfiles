...
PRIMARY_WIDTH="1920"
PRIMARY_HEIGHT="1080"
SECONDARY_MONITOR=$( hyprctl monitors | awk '($1 == "Monitor") {print $2}' | tail -n 1 | rofi -dmenu -p "Please select a display source!")
SECONDARY_WIDTH=$(hyprctl monitors | grep "${SECONDARY_MONITOR}" -A 1 | tail -n 1 | awk '{print $1}' | cut -d 'x' -f 1)
SECONDARY_HEIGHT=$(hyprctl monitors | grep "${SECONDARY_MONITOR}" -A 1 | tail -n 1 | awk '{print $1}' | cut -d 'x' -f 2 | cut -d '@' -f 1)
SCALING_FACTOR="1.5"
...
ANSWER=$(echo -e "${WOFI_OPTIONS}" | rofi -dmenu -p "How do you want to project?" |
tr '[:upper:]' '[:lower:]')
[ -z "${ANSWER}" ] && {
 echo "No option is selected!"
 exit 1
